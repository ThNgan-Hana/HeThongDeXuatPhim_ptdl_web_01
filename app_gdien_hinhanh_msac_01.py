...
# ==============================================================================
# 0. CẤU HÌNH TRANG & CSS (MÀU SẮC TỰ ĐỘNG THÍCH ỨNG)
# ==============================================================================
st.set_page_config(
...
def inject_custom_css():
    st.markdown("""
    <style>
    /* 1. TỐI ƯU KHOẢNG CÁCH */
    .block-container {
        padding-top: 1.5rem !important;
        padding-bottom: 3rem !important;
    }

    /* 2. CARD PHIM (SỬ DỤNG BIẾN MÀU HỆ THỐNG) */
    .movie-card-container {
        /* Tự động đổi màu xám nhạt (Light) hoặc xám đậm (Dark) */
        ...
        height: 100%;
        color: var(--text-color); /* Chữ tự động đổi màu */
    }
    ...
    /* 4. NÚT BẤM (MÀU ĐỎ THƯƠNG HIỆU - ĐẸP TRÊN CẢ 2 NỀN) */
    .stButton > button {
        background-color: #E50914 !important; /* Đỏ Netflix */
    ...
    .small-text {
        font-size: 0.85rem;
        opacity: 0.7; /* Làm mờ nhẹ thay vì chọn màu cứng */
    }

    /* 6. INPUT FIELDS */
    /* Làm trong suốt nền input để ăn theo màu hệ thống */
...
def load_and_preprocess_static_data():
    try:
        ...
        df["combined_features"] = (df["Đạo diễn"].astype(str) + " " + df["Diễn viên chính"].astype(str) + " " + df[
            "Thể loại phim"].astype(str))
...
def display_movie_grid(df_result, title=None):
    ...
    for index, (i, row) in enumerate(df_result.iterrows()):
        with cols[index % 5]:
            # Container border=False vì ta đã style border bằng CSS .movie-card-container
            with st.container():
                # Bọc nội dung trong thẻ div có class custom để nhận CSS
                ...
                if isinstance(poster, str) and poster.startswith('http'):
                    ...
                    # Placeholder màu xám trung tính
                    st.markdown(
                        f"""<div style="background:rgba(128,128,128,0.2);height:380px;display:flex;align-items:center;justify-content:center;border-radius:8px;color:var(--text-color);">No Image</div>""",
                        unsafe_allow_html=True)

                # 2. Info Compact
                st.markdown(f"#### {row['Tên phim']}")
                st.markdown(f"<div class='small-text'>📅 {row.get('Năm phát hành', 'N/A')}</div>",
                            unsafe_allow_html=True)
...
def get_recommendations_weighted_genres(selected_genres, df, num=10):
    ...
    filtered['final_score'] = filtered.apply(
        lambda x: x['popularity_norm'] * 2 + sum(1 for g in selected_genres if g in str(x['Thể loại phim'])) * 1.5 + x[
            'recency_score'], axis=1)
...
def main_page(df_movies, cosine_sim, sorted_genres):
    ...
    if is_guest:
        ...
        if genres:
            ...
            display_movie_grid(df_movies.sort_values(['year_numeric', 'popularity_norm'], ascending=False).head(10),
                               "🔥 Phim Mới & Hot")
        return
    ...
    if menu == 'Tìm kiếm':
        ...
        with c1:
            selected_movie = st.selectbox("Chọn phim", get_unique_movie_titles(df_movies), label_visibility="collapsed",
        ...
        if search_btn:
            ...
            display_movie_grid(df_movies.sort_values('popularity_norm', ascending=False).head(10), "🎬 Đề xuất hôm nay")
            ...
            with st.form("reg"):
                ...
                if st.form_submit_button("Đăng Ký", type="primary", use_container_width=True):
                    ...
                    if new_u and (users.empty or new_u not in users['Tên người dùng'].values):
                        row = {'ID': len(users) + 1, 'Tên người dùng': new_u, '5 phim coi gần nhất': "[]",
                               'Phim yêu thích nhất': fav, 'Thể loại yêu thích': ",".join(g)}
...
if __name__ == '__main__':
    ...
    if st.session_state['logged_in_user']:
        main_page(df, sim, genres)
    else:
        authentication_page(df, genres)